...
where T:ServiceLayer<Config=In,Output=Out> ,
    In:for<'a> Deserialize<'a> + Send + Sync,
    Out:Serialize + Send + Sync
{
    async fn call(&self, flow: Flow) -> anyhow::Result<Output> {
        let Flow{ ctx, code, node_config,node_type_id, .. } = flow;
        let node_info = format!("task[{}],node_type_id[{}],code[{}]",ctx.code,node_type_id,code);
        let cfg = match serde_json::from_str::<In>(node_config.as_str()){
            Ok(o)=>o,
            Err(e)=>{
                return anyhow::anyhow!("code[{}],type parse failed, data[{}] error:{}",node_info,node_config.as_str(),e).err()
            }
        };
        ...
        let raw = match serde_json::to_value(&output){
            Ok(o)=>o,
            Err(e)=>{
                return anyhow::anyhow!("code[{}],output json error:{}",node_info,e).err()
            }
        };
...
impl<T,In,Out> LayerJson<T,In,Out>  {
    pub fn new(handle:T)->Self{
        LayerJson{handle,_in:PhantomData::default(),_out:PhantomData::default()}
    }
...
where T:Fn(String,Arc<Context>,In)->Fut + Send + Sync,
...
impl<T,In,Out,Fut> LayerJsonFn<T,In,Out,Fut> {
    pub fn new(function:T)->Self{
        let inner = LayerJson::new(function);
        Self{inner,_fut:PhantomData::default()}
    }
}

impl Runtime{
    pub fn register_service_layer<ID:Into<String>,T,I,O,F:Into<LayerJson<T,I,O>>>(self,id:ID,layer_fn:F)->Self
    where T:ServiceLayer<Config=I,Output=O>+ 'static,
...
mod test{
    ...
    use crate::{Context, END_NODE_CODE, LayerJson, Output, PlanBuilder, Runtime, ServiceLayer};
    ...
    impl ServiceLayer for LayerTest{
        ...
        async fn call(&self,_:String, ctx: Arc<Context>, cfg: Self::Config) -> anyhow::Result<Self::Output> {
            println!("config ---> {}",cfg);
            Value::String("success".into()).ok()
        }
    }

    async fn service_layer_fn_show(_:String, ctx: Arc<Context>, cfg:Value) -> anyhow::Result<Value> {
    ...
    pub async fn test_layer_json(){
        let rt = Runtime::default()
            ...
            .register_service_layer("lambda_fn",|code:String,ctx:Arc<Context>,_:Value|async move{
                println!("lambda_fn code ---> {}",code);
                Value::String("lambda success".into()).ok()
            })
            .launch();

        let plan = PlanBuilder::start((END_NODE_CODE,"layer_test",r#"{"key":"hello"}"#),vec![""]).check_and_build().unwrap();
        let res = rt.ctx("test001", plan).arc().block_on::<Value>().await.unwrap();
        println!("{:?}", res.to_string());
        assert_eq!(r#""success""#,res.to_string());

        let plan = PlanBuilder::start((END_NODE_CODE,"layer_fn",r#"{"key":"world"}"#),vec![""]).check_and_build().unwrap();
        let res = rt.ctx("test001", plan).arc().block_on::<Value>().await.unwrap();
        println!("{:?}", res.to_string());
        assert_eq!(r#""fn success""#,res.to_string());

        let plan = PlanBuilder::start((END_NODE_CODE,"lambda_fn",r#"{}"#),vec![""]).check_and_build().unwrap();
        let res = rt.ctx("test001", plan).arc().block_on::<Value>().await.unwrap();
        println!("{:?}", res.to_string());
