...
impl PlanBuilder{
    ...
    pub fn insert_node<N:Into<PlanNode>>(&mut self, node:N) ->&mut Self{
        ...
        self.map.insert(code,node);self
    }
    pub fn sequence<N:Into<Node>,S:Into<String>>(&mut self, nodes:Vec<N>, next_node_code:S) ->&mut Self{
    ...
    pub fn merged<R:Into<String>,S:Into<String>,N:Into<Node>>(&mut self,ready_codes:Vec<R>,node:N,next_node_code:S) ->&mut Self{
        let ready_codes = ready_codes.into_iter().map(|x|x.into()).collect::<Vec<String>>();
        self.insert_node((ready_codes,node.into(),vec![next_node_code.into()]))
    }
    pub fn end<R:Into<String>,N:Into<Node>>(&mut self,ready_codes:Vec<R>,node:N) ->&mut Self{
        let ready_codes = ready_codes.into_iter().map(|x|x.into()).collect::<Vec<String>>();
...
mod test{
    ...
    use crate::{END_NODE_CODE, Node, START_NODE_CODE};
