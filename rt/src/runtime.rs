...
impl RuntimeWait{
    pub fn new(code:String,callback:Arc<dyn CallBackSet>,output: Arc<Mutex<Option<TaskOutput>>>)->Self{
        RuntimeWait{code,callback,output}
    }
}
...
impl Runtime{
    pub fn new()->Self{
        ...
        Self{
            status,nodes,task_chan,result_chan,callback
        }
    }
    ...
    pub fn launch(&mut self){
        ...
        tokio::spawn(async {
            rt.start_work(task_receiver,task_sender,result_sender).await;
        });
        let rt = self.clone();
        ...
        tokio::spawn(async move {
            ...
            while let Ok(output) = result_chan.recv().await{
                ...
                if let Some(call) = self.callback.remove(code.code.as_str()) {
                    ...
                    wd_log::log_field("error","not find task waker").field("task_flow_code",code.code.as_str()).info("not find");
                }
            }
        });
    }

    async fn start_work(self, receiver:Receiver<Task>, task_sender:Sender<Task>, result_chan:Sender<TaskOutput>){
            ...
            let node = match node_loader.get(id.as_str()) {
                ...
                Err(e) => {
                    ...
                    if let Err(e) = result_chan.send(output).await{
                        wd_log::log_field("error",e).error("get node failed,send to result_chan channel failed");
                    }
                    continue
                }
            };
    ...
    fn async_run_task(&self, task:Task, node:Arc<dyn Node>, task_chan:Sender<Task>, result_chan:Sender<TaskOutput>){
        let Task{
            ctx,node_id,input
        } = task;
        tokio::spawn(async move{
            ...
            let mut output = match result{
                ...
                Err(e) => {
                    ...
                    output.set_ctx(ctx);
                    if let Err(e) = result_chan.send(output).await{
                        wd_log::log_field("error",e).error("node run failed, send to result_chan channel failed");
                    }
                    return;
                }
            };
            if output.over || output.error.is_some(){
                ...
                if let Err(e) = result_chan.send(output).await{
                    wd_log::log_field("error",e).error("node over, send to result_chan channel failed");
                }
                return;
            }
            for (id,val) in output.result{
                ...
                if let Err(e) = task_chan.send(task).await{
                    wd_log::log_field("error",e).error("run next node, send to task_chan channel failed");
                }
                return;
            }
        });
