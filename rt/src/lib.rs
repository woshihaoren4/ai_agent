...
mod tests {
    use wd_tools::PFArc;
    use crate::{END_NODE_CODE, Output, PlanBuilder, Runtime};
    ...
    pub async fn test_runtime_stack(){
        ...
        let plan = PlanBuilder::start(("A","node_id_1"),vec!["B"])
            .sequence(vec![("B", "node_id_1",""),("C","node_id_1","")],END_NODE_CODE)
    ...
    pub async fn test_runtime_one(){
        ...
        let plan = PlanBuilder::start((END_NODE_CODE,"node_id_1"),vec![""]).check_and_build().unwrap();
    ...
    pub async fn test_runtime_sequence(){
        ...
        let plan = PlanBuilder::start(("A","node_id_1"),vec!["B"])
            .sequence(vec![("B", "node_id_2",""),(END_NODE_CODE,"node_id_3","")],"")
            .check_and_build().unwrap();
    ...
    pub async fn test_runtime_sub_task(){
        ...
        let out = rt.ctx("test_runtime_sub_task", plan).arc().block_on::<String>().await.unwrap();
        println!("--->{}",out);
    }
}
