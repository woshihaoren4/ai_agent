...
impl Client {
    ...
    #[allow(dead_code)]
    pub async fn call_function(&self, req:CallFunctionRequest) ->anyhow::Result<CallFunctionResponse>{

        self.pool.defer(move |mut conn|async move{
            let result = conn.deref_mut().call_function(req).await;

            let resp = match result {
                Ok(o) => o.into_inner(),
                Err(e) => {
                    return anyhow::anyhow!("Client.call_function error:{}",e.to_string()).err()
                }
            };
    ...
    #[allow(dead_code)]
    pub async fn raw_eval<C,M,F,S,Args,Output>(&self,ty:SrcType,code:Option<C>,module_name:M,sys_path:Option<S>,function_name:F,args:Args)->anyhow::Result<Output>
    where C:Into<String>,M:Into<String>,F:Into<String>,S:Into<String>,Args:Serialize,Output:for <'a> Deserialize<'a>,
    {
        ...
        if out.code != 0 {
            return anyhow::anyhow!("raw_eval_script_code failed code[{}] msg[{}]",out.code,out.msg).err()
        }
        if let Some(s) = out.output {
            ...
            let out = match serde_json::from_value(value){
                Ok(o)=>o,
                Err(e)=>{
                    return anyhow::anyhow!("raw_eval_script_code failed parse output error:{}",e).err()
                }
            };
            return Ok(out)
        }
    ...
    #[allow(dead_code)]
    pub async fn eval_script_code<C:Into<String>,F:Into<String>,Args:Serialize,Output:for <'a> Deserialize<'a>>(&self,code:C,function_name:F,args:Args)->anyhow::Result<Output>{
        self.raw_eval(SrcType::Script, Some(code),"default_module",None::<String>,function_name,args).await
    }
    #[allow(dead_code)]
    pub async fn eval_module<C:Into<String>,S:Into<String>,F:Into<String>,Args:Serialize,Output:for <'a> Deserialize<'a>>(&self,sys_path:S,module_name:C,function_name:F,args:Args)->anyhow::Result<Output>{
        self.raw_eval(SrcType::Module,None::<String>,module_name,Some(sys_path),function_name,args).await
    }
}
