...
impl SelectorService {
    pub fn judge(vars: &mut VecDeque<Value>) -> anyhow::Result<bool> {
        ...
        let comparator = if let Some(s) = vars.pop_front() {
            if let Value::String(s) = s {
                ...
                return anyhow::anyhow!(
                    "comparator must is string[==,!=,>,>=,<,<=,is_null,no_null,contain,no_contain]"
                )
                .err();
            }
        } else {
        ...
        return match comparator.as_str() {
            ...
            // todo "contain"=>{}
            // todo "no_contain"=>{}
            _ => anyhow::anyhow!("unknown comparator[{}]", comparator).err(),
        };
    }
}
...
mod test {
    ...
    pub async fn test_selector_service() {
        ...
        let plan = PlanBuilder::start::<_, String>(
            (
                "select-01",
                "flow_chart_selector",
        ...
        .fission_from_code(
            ...
            (
                "branch_001",
                "flow_chart_var",
                r#"{"result":"select node branch_001"}"#,
            ),
            vec![END_NODE_CODE],
        )
        .fission_from_code(
            ...
            (
                "branch_002",
                "flow_chart_var",
                r#"{"result":"select node branch_002"}"#,
            ),
            vec![END_NODE_CODE],
        )
        .end::<String, _>(
            ...
            (
                "end",
                "flow_chart_var",
                r#"{"result":"{{branch_001.result}}{{branch_002.result}}"}"#,
            ),
        )
        .build();

        let output = rt
            .ctx("selector-test-001", plan)
            .arc()
