...
impl InjectorService {
    pub fn operate(src:&mut Value, mut value:Value, operate:String) ->anyhow::Result<()>{
        match operate.to_lowercase().as_str() {
            ...
            _=>{
                return anyhow::anyhow!("InjectorService.operate not support operate[{}]",operate).err()
            }
        }
        Ok(())
    }
    pub fn update(pos:String,ctx: &Context,function:impl FnOnce(&mut Value)->anyhow::Result<()> + 'static)->anyhow::Result<()>{
        let mut ks = pos.as_str().split(".").collect::<VecDeque<&str>>();
        let code = ks.pop_front().unwrap_or("").to_string();
        return ctx.plan.update(code.as_str(),Box::new(move |x|{
            ...
            let node = if let Some(ref mut s) = plan.cfg {
                ...
                return anyhow::anyhow!("InjectorService.update node[{}] cfg is null",code).err()
            };
            let mut cfg = serde_json::from_str::<Value>(node.node_config.as_str()).unwrap_or(Value::Null);
...
impl agent_rt::ServiceLayer for InjectorService {
    ...
    async fn call(&self, _code: String, ctx: Arc<Context>, cfg: Self::Config) -> anyhow::Result<Self::Output> {
        ...
        Self::update(to,&ctx,|x|{
            Self::operate(x,from,operate)
        })?;
...
mod test{
    ...
    async fn test_injector(){
        ...
        let result = rt.ctx("injector-001", PlanBuilder::start(("injector", "flow_chart_injector", serde_json::to_string(&cfg).unwrap()), vec![END_NODE_CODE])
            .end::<String, _>(vec![], ("end", "flow_chart_var", r#"{"output":["first"]}"#)).build())
            .arc()
