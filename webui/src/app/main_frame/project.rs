        ...
        egui::SidePanel::left("project")
            ...
            .show_animated(ctx, open, |ui| {
                ...
                egui::Grid::new("project")
                    ...
                    .show(ui, |ui| {
                        ui.label("serve addr:")
                            .on_hover_text("default: http://127.0.0.1:50000");
                        ui.text_edit_singleline(&mut cfg.project_cfg.server_addr);
                        ui.end_row();

                        ui.label("update plugin_view:");
                        if ui.button("LOAD").clicked() {
                            cfg.plugin.tools_loader = infra::get_json(
                                format!("{}/api/v1/plugin", cfg.project_cfg.server_addr).as_str(),
                            );
                        }
                        match cfg
                            .plugin
                            ...
                            .try_get_json::<BTreeMap<String, Vec<PluginService>>>()
                        {
                            ...
                            Some(Err(e)) => {
                                cfg.debug_win.info(format!("load plugin_view error:{e}"));
                            }
                        }
                        ui.end_row();
