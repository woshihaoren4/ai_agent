                        ...
                        let context = WorkFlowDebugRequest::new(&cfg.plugin.nodes)
                            .map(|req| {
                                serde_json::to_string_pretty(&req)
                                    .unwrap_or_else(|err| format!("json plan error:{}", err))
                            })
                            .unwrap_or_else(|err| format!("generate plan error:{}", err));
                        cfg.options_view.text_view_content = context;
                    }
                    if ui.button("down plugin").clicked() {
                        let s = serde_json::to_string(&cfg.plugin.nodes)
                            .unwrap_or_else(|e| e.to_string());
                        cfg.options_view.text_view_content = s;
                    }
                    if ui.button("up plugin").clicked() {
                        match serde_json::from_str::<BTreeMap<String, PluginServiceWin>>(
                            cfg.options_view.text_view_content.as_str(),
                        ) {
                            Ok(o) => {
                                cfg.plugin.nodes = o;
