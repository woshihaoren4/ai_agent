...
impl Debug {
    pub fn debug_workflow(ctx: &Context, ui: &mut egui::Ui, cfg: &mut State) {
            ...
            .clicked()
        {
            if !cfg.plugin.debug_loader.is_waiting() {
                ...
                match WorkFlowDebugRequest::new(&cfg.plugin.nodes) {
                    Ok(body) => {
                        cfg.debug_win
                            .debug(serde_json::to_string(&body).unwrap_or("".into()));
                        let result = infra::post_json_stream(
                            format!("{}/api/v1/agent/call", cfg.project_cfg.server_addr).as_str(),
                            &body,
                            |x| x,
                        );
                        match result {
                    ...
                    Err(e) => {
                        cfg.debug_win
                            .error(format!("debug request parse error:{:?}", e));
                    }
                }
            }
        }
        ...
        match cfg.plugin.debug_loader.try_get_string() {
            None => {}
            Some(Ok(o)) => {
                    ...
                    Ok(resp) => {
                        if let Some(ref node) = resp.result {
                            if let Some(n) = cfg.plugin.nodes.get_mut(node.node_code.as_str()) {
                                ...
                                cfg.debug_win
                                    .warn(format!("not found node win[{}]", node.node_code));
                            }
                        } else {
                            let text =
                                format!("debug response result is null! \n -> output <-\n{}", o);
    ...
    fn update(&mut self, ctx: &Context, _frame: &mut Frame, cfg: &mut State) {
        if cfg.debug_win.open {
            ...
            egui::Window::new("DebugInfoDetail")
                ...
                .show(ctx, |ui| {
                    egui::TextEdit::multiline(&mut info).show(ui);
                });
        } else {
...
impl WorkFlowDebugPlanNode {
    pub fn copy_from_service(&mut self, win: &PluginServiceWin) -> anyhow::Result<()> {
        ...
        self.cfg =
            serde_json::to_string(&Self::cfg_from_service(&win.service)?).unwrap_or("".into());
