...
impl TopControlTools{
    ...
    pub fn ui(ctx: &Context, cfg: &mut State) {
        ...
        let mut node_list = cfg.plugin.nodes.iter().map(|x|x.0.clone()).collect::<Vec<_>>();

        for (name,node) in cfg.plugin.nodes.iter_mut(){
            let PluginServiceWin { open, pos, service, goto, goto_select,.. } = node;
            ...
            let resp = egui::Window::new(name)
                ...
                .show(ctx, |ui| {
                    ...
                    ui.horizontal_top(|ui|{
                        ...
                        ui.label(egui::WidgetText::RichText(egui::RichText::new(service.service_type.as_str())).color(egui::Color32::BLUE));
                    });
                    //渲染入参
                    ui.separator();
