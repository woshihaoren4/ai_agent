...
use wd_tools::{EncodeHex, MD5, PFErr, PFOk};
use bytes::Buf;
...
impl DashVector {
    ...
    async fn recall(&self,req:DashVectorRecallRequest)->anyhow::Result<Vec<String>>{
        let req = Request::post(format!("https://{}/v1/collections/summery_coll/query",self.cluster))
            .header("Content-Type","application/json")
            .header("dashvector-auth-token",self.api_key.clone())
            .body(Self::full(serde_json::to_string(&req)?)).unwrap();
    ...
    async fn insert_api(&self, req: DashVectorCreateRequest) ->anyhow::Result<()>{
        let req = Request::post(format!("https://{}/v1/collections/summery_coll/docs",self.cluster))
            .header("Content-Type","application/json")
            .header("dashvector-auth-token",self.api_key.clone())
            .body(Self::full(serde_json::to_string(&req)?)).unwrap();
        let resp = self.client.request(req).await?;
...
mod test{
    ...
    async fn test_hyper_https(){
        ...
        let resp = client.get("https://www.baidu.com".parse().unwrap()).await.unwrap();
        println!("resp-->{:?}",resp);
    }
    ...
    async fn test_insert_api(){
        ...
        dv.insert("001".to_string(),vec!["我叫王大锤".to_string()]).await.unwrap();
    }
    #[tokio::test]
    async fn test_recall(){
        let dv = DashVector::new();
        let item = dv.top_n("001".into(), "我叫王大锤".into(), 1).await.unwrap();
        println!("{:?}",item)
    }
}