...
use async_openai::types::{ChatCompletionRequestMessage, ChatCompletionRequestSystemMessageArgs, ChatCompletionRequestUserMessageArgs, ChatCompletionTool, CreateChatCompletionRequestArgs, CreateChatCompletionResponse};
    ...
    pub async fn chat(&self,req:LLMNodeRequest) ->anyhow::Result<CreateChatCompletionResponse>{
        ...
        if !prompt.is_empty() {
            if let Some(p) = context.get_mut(0){
                if let ChatCompletionRequestMessage::System(pe) = p{
                    // pe.content = {
                    //     prompt.push_str("\r\n");
                    ...
                    context.insert(0,ChatCompletionRequestSystemMessageArgs::default().content(prompt).build().unwrap().into())
                }
            }else{
                context.insert(0,ChatCompletionRequestSystemMessageArgs::default().content(prompt).build().unwrap().into())
            }
        ...
        if !query.is_empty() {
            context.push(ChatCompletionRequestUserMessageArgs::default().content(query).build().unwrap().into());
        }
        ...
        let resp = chat.create(req).await?;Ok(resp)
    }
}
...
mod test{
    ...
    use async_openai::types::{ChatCompletionRequestAssistantMessageArgs, ChatCompletionRequestMessage, ChatCompletionRequestUserMessageArgs, CreateChatCompletionResponse};
    use wd_tools::PFArc;
    use rt::{Context, TaskInput, TaskOutput};
    ...
    async fn test_llm_node(){
        ...
        while let Some(Ok(query)) = stdin.next(){
            ...
            let mut resp:TaskOutput = llm.go(Context::new("test").arc(), TaskInput::from_value(req)).await.unwrap();
            ...
            ctx.push(ChatCompletionRequestAssistantMessageArgs::default()
                .content(answer)
                .build().unwrap().into());
