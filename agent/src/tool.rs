...
pub struct ToolNode{
    ...
    handle:Box<dyn Fn(String)->Pin<Box<dyn Future<Output=anyhow::Result<String>>+Send>>+Send+Sync+'static>
}
impl ToolNode{
    pub fn new<S:Into<String>,D:Into<String>,F>(id:S,desc:D,param:&str, handle:F)->Self
    where F:Fn(String)->Pin<Box<dyn Future<Output=anyhow::Result<String>>+Send>>+Send+Sync+'static
    {
        let id = id.into();
...
impl ToolNode {
    pub(crate) fn mock_get_current_weather()->Self{
        ToolNode::new("get_current_weather",
    ...
    pub(crate) fn mock_taobao()->Self{
        ToolNode::new("submit_order",
                      "在线购物",
                      r#"{"type":"object","properties":{"product":{"type":"string","description":"商品名称"}},"required":["product"]}"#,
                      Self::submit_order)
    }
    fn get_current_weather(input:String)->Pin<Box<dyn Future<Output=anyhow::Result<String>>+Send>>{
        Box::pin(async move{
            ...
            let location = if let Value::String(s)=location{s}else{
    ...
    async fn go(&self, ctx: Arc<Context>, mut args: TaskInput) -> anyhow::Result<TaskOutput> {
        ...
        if let Some(call) = args.get_value::<ChatCompletionMessageToolCall>(){
            ...
            let msg:ChatCompletionRequestMessage = ChatCompletionRequestToolMessageArgs::default()
                ...
                .content(resp).build().unwrap().into();
...
#[cfg(test)]
mod test{
    use async_openai::types::{ChatCompletionMessageToolCall, ChatCompletionRequestMessage, FunctionCall};
    use wd_tools::{PFArc};
    ...
    async fn test_tool(){
        ...
        let mut x = tool.go(Context::new("get_current_weather").arc(), TaskInput::from_value(r#"{"location":"shanghai"}"#.to_string())).await.unwrap();
    ...
    async fn test_chat_msg(){
        ...
        let req = ChatCompletionMessageToolCall{
            ...
            function: FunctionCall{ name: "get_current_weather".to_string(), arguments: r#"{"location":"shanghai"}"#.to_string() },
        };

        let mut x = tool.go(Context::new("get_current_weather").arc(), TaskInput::from_value(req)).await.unwrap();
